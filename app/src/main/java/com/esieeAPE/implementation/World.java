                ...
                ((Activity) World.this.context).runOnUiThread(new Runnable() {
                    public void run() {
                        if (MainActivity.config.getControlMode() == Const.MODE_SHUTTLE)
                            if (tmp < 0.0025)
                                Toast.makeText(World.this.context, "LANDING : Shuttle Contact Velocity dY :" + tmp, Toast.LENGTH_SHORT).show();
                            else
                                Toast.makeText(World.this.context, "CRASH : Shuttle Contact Velocity dY :" + tmp, Toast.LENGTH_SHORT).show();
                    }
                });
            }
