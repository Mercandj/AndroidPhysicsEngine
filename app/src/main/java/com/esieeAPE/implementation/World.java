...
/**
 * SPECIFIC mWorld : Place and define all objects
 *
 * @author Jonathan
 */
public class World extends EntityGroup {
    ...
    public World(final Context context, final Camera camera) {
        mContext = context;
        mCamera = camera;
    }
    ...
    public void init() {
        ...
        car_camaro.mTexture = new Texture(mContext, R.drawable.color_white);
        ...
        for (int z = 0; z < step_apple; z++) {
            for (float i = -borne_apple; i <= borne_apple; i++) {
                for (float j = -borne_apple; j <= borne_apple; j++) {
                    ...
                    if (z % 3 == 0) {
                        apple.mTexture = new Texture(mContext, R.drawable.color_green);
                    } else if (z % 3 == 1) {
                        apple.mTexture = new Texture(mContext, R.drawable.color_red);
                    } else {
                        apple.mTexture = new Texture(mContext, R.drawable.color_white);
                    }
        ...
        for (int i = 0; i < tmp_width; i++) {
            for (int j = 0; j < tmp_height; j++) {
                ...
                if (j % 3 == 0) {
                    point.mTexture = new Texture(mContext, R.drawable.color_green);
                } else if (j % 3 == 1) {
                    point.mTexture = new Texture(mContext, R.drawable.color_red);
                } else {
                    point.mTexture = new Texture(mContext, R.drawable.sky2);
                }
                if (j != 0) {
        ...
        shuttle.mTexture = new Texture(mContext, R.drawable.color_white);
        ...
        apple.mTexture = new Texture(mContext, R.drawable.shingles);
        apple.mTextureBumpMapping = new Texture(mContext, R.drawable.shingles_bump, 1);
        apple.mPhysic.mass = 0.11f;
        addEntity(apple);
        ...
        bwall.mTexture = new Texture(mContext, R.drawable.wall);
        addEntity(bwall);
        ...
        house.mTexture = new Texture(mContext, R.drawable.floor);
        ...
        */
        ...
        floor.mTexture = new Texture(mContext, R.drawable.floor);
        addEntity(floor);


        /********* INIT FORCES *********/

        addForce(ENUM_Forces.GRAVITY.force);
        ...
        ENUM_Forces.GRAVITY_UP.force.isApplyForce = new Predicate() {
            ...
            public Vector3D isTrue(Entity entity) {
				...
				*/
                ...
                if (entity.mPosition.dY < 50 &&
                        PhysicsEngine.actionButton &&
                        MainActivity.config.getControlMode() == Const.MODE_JUMP &&
                        shuttleId != entity.mId &&
                        carId != entity.mId &&
                        bumpAppleId != entity.mId) {
        ...
        addForce(ENUM_Forces.GRAVITY_UP.force);

        Predicate p_shuttle = new Predicate() {
            @Override
            public Vector3D isTrue(Entity entity) {
                if (shuttle.mId == entity.mId &&
                        PhysicsEngine.actionButton &&
                        MainActivity.config.getControlMode() == Const.MODE_SHUTTLE) {
                    return new Vector3D(0, 1.1f, 0);
                }
                return new Vector3D(0, 0, 0);
            }
        };
        addForce(new Force(0.000004f, true, p_shuttle));

        Predicate p_car_camaro = new Predicate() {
            @Override
            public Vector3D isTrue(Entity entity) {
                if (car_camaro.mId == entity.mId &&
                        PhysicsEngine.actionButton &&
                        MainActivity.config.getControlMode() == Const.MODE_CAR) {
                    return new Vector3D(car_camaro.forward.dX, car_camaro.forward.dY, car_camaro.forward.dZ);
                }
                return new Vector3D(0, 0, 0);
            }
        };
        addForce(new Force(0.000003f, true, p_car_camaro));
    }
}
